import React, { useEffect, useRef, useState } from "react";
import Footer from "../footer";
import Sidebaar from "./Sidebaar";
import {
  mapStateToProps,
  mapDispatchToProps,
} from "../../src/constants/contextProvider";
import { ToastContainer, toast } from "react-toastify";
import { Popconfirm } from "antd";
import MainContainer from "../components/Container";
import { connect, useDispatch, useSelector } from "react-redux";
import { useRouter } from "next/router";
import axios from "axios";
import { MethodNames } from "../../src/constants/methodNames";
import { GetUserId, baseUrl } from "../../src/constants/constants";
import { LoginUserId, stateMasterApi } from "../api/AllApi";
import moment from "moment/moment";
import { Modal, Button } from "react-bootstrap";
import Vaildation from "./Vaildation";
import ErrorPage from "./ErrorPage";

const SalaryBreakup = (props) => {
  const open = useSelector((state) => state.projectR.sideClick);
  const show = useSelector((state) => state.projectR.showProject);
  const router = useRouter();
  //  get projectid and emp id on create new employee
  const { ProjectId, SalaryBreakupData, IsPFDeducted, IsESIDeducted } = router.query;
  const AddData = { Projectid: "", EmpId: "" };
  const [GetProjectId, setGetProjectId] = useState(AddData);
  const [ViewProjectPayComponent, setViewProjectPayComponent] = useState("");
  const [SalaryBreakup, setSalaryBreakup] = useState([]);
  const [UserId, setUserId] = useState("");


//  window back page clear  code
  useEffect(() => {
    const clearHistory = () => {
      window.history.pushState(null, '', window.location.href);
      window.onpopstate = function () {
        window.history.pushState(null, '', window.location.href);
      };
    };

    clearHistory();

    return () => {
      window.onpopstate = null; // Cleanup on component unmount
    };
  }, []);

  // user id get in login
  useEffect(() => {
    const UserId = async () => {
      const userid = await LoginUserId();
      setUserId(userid);
    };
    UserId();
  }, []);

  const [SalaryBreakupEsiPf, setSalaryBreakupEsiPf] = useState({
    PFDeduct: 0,
    ESIDeduct: 0,
  });

  const [Render, setRender] = useState(false);

  //  get projectid and emp id on update new employee
  const EditSalaryBreakups = router.query.EditSalaryBreakup ? JSON.parse(router.query.EditSalaryBreakup): {};

    console.log("EditSalaryBreakup-------------0000",EditSalaryBreakups)





// RENDER DATA EDIT GET PROJECTID & EMPID IsPFDeducted & IsESIDeducted DATA 
  useEffect(() => {
    if (Object.keys(EditSalaryBreakups).length > 0) {
      setGetProjectId(EditSalaryBreakups);
      let esiData = {PFDeduct: EditSalaryBreakups.IsPFDeducted == true ? 1 : 0,
        ESIDeduct: EditSalaryBreakups.IsESIDeducted == true ? 1 : 0,
        };
        console.log("esiData",esiData)
    
        setSalaryBreakupEsiPf(esiData);
    } else if (
      SalaryBreakupData != "" &&
      SalaryBreakupData != null &&
      SalaryBreakupData != undefined
    ) {
      const AddData = {
        Projectid: ProjectId,
        EmpId: SalaryBreakupData,
      };
      
      console.log("NewEmployee-----56", AddData);
      setGetProjectId(AddData);
    }
  }, [Object.keys(EditSalaryBreakups).length > 0]);

  //   View Salary Structure  Code

  const ViewProjectPayComponents = async () => {
    const ProjectPayComponent = {
      ProjectId: ProjectId || GetProjectId.Projectid,
      EmpId: GetProjectId.EmpId,
    };
    await axios
      .post(baseUrl + MethodNames.ViewProjectPayComponent, ProjectPayComponent)
      .then((res) => {
        console.log(
          "response ViewProjectPayComponent DATA----",
          res.data.ViewProjectPayComponent
        );
        setViewProjectPayComponent(res.data.ViewProjectPayComponent);
        FilterDataSalaryStr(res.data.ViewProjectPayComponent);
      });
  };

  //    fliter check data on view salary str
  const FilterDataSalaryStr = (EditSalaryBreakupSelect) => {
    const EditSalaryBreakupObj = EditSalaryBreakupSelect.filter((obj) => obj.PayCompValue > 0);
    let newArrObj = EditSalaryBreakupObj.map((item) => {
      return {
        EmpId: item.EmpId,
        PayCompCode: item.PayCompCode,
        PayCompValue: item.PayCompValue,
      };
    });
    console.log("newArrObj-----------", newArrObj);
    setSalaryBreakup(newArrObj);
  };

  //  render data
  useEffect(() => {
    if (GetProjectId &&GetProjectId.ProjectId != "" &&GetProjectId.EmpId != "") {
      ViewProjectPayComponents();
      console.log("GetProjectId updatwe----", GetProjectId.ProjectId,GetProjectId.EmpId);
    }
  }, [GetProjectId.ProjectId, GetProjectId.EmpId]);

  //  Add Salary Structure  Code

  const handleSubmit = async (e) => {
    e.preventDefault();
    let val = await Vaildation(SalaryBreakupEsiPf);
    if (!val) {
      return;
    }
    console.log("added selected SalaryBreakup",SalaryBreakup,SalaryBreakupEsiPf)
    if(SalaryBreakup.length<=0){
      toast.error("please select Salary Breakup")
      return
    }
 

    const payload = {
      EmpPFESIDeduction: [
        {
          EmpId: GetProjectId.EmpId,
          PFDeduct: SalaryBreakupEsiPf.PFDeduct,
          ESIDeduct: SalaryBreakupEsiPf.ESIDeduct,
          UserId: UserId,
        },
      ],
      EmpSalaryStructure: SalaryBreakup,
    };
    console.log("payload-----3-", payload);
    
    const UpsertMaster = JSON.stringify(payload);
    const SalaryStructure = {
      OperationType: "Add",
      JsonData: UpsertMaster,
    };
    console.log("payload------4", SalaryStructure);
    const UpsertEmployeeOfferSalaryStructure = await axios
      .post( baseUrl + MethodNames.UpsertEmployeeOfferSalaryStructure,SalaryStructure)
      .then((res) => {
        console.log("payload------ sucess", res.data);

        if (res.data.UpsertEmployeeOfferSalaryStructure) {
          toast.success("Salary Breakup added  successfully ");
          router.push(
            {
              pathname: "/Screen/EmpOfferLetter",
              query: {
                //  SalaryBreakupData: JSON.stringify(res.data.UpsertMasterEmployee[0].RecordStatus),
                EmpIdOfferLetter: GetProjectId.EmpId,
              },
            },
            undefined,
            { shallow: true }
          );
          // router.push("/Screen/EmployeeDetails");
        }
      });
    //  ViewProjectPayComponents()
  };

  //  Update Salary Structure  Code
  const handleUpdate = async (e) => {
    e.preventDefault();
    const LoginUserIds = await sessionStorage.getItem("UserDetails")
    let val = await Vaildation(SalaryBreakupEsiPf);
    if (!val) {
      return;
    }

    console.log("updated selected SalaryBreakup",SalaryBreakup,SalaryBreakupEsiPf)
    if(SalaryBreakup.length<=0){
      toast.error("Please select salary breakup ")
      return
    }
  const payload = {
      EmpPFESIDeduction: [
        {
          EmpId: GetProjectId.EmpId,
          PFDeduct: SalaryBreakupEsiPf.PFDeduct,
          ESIDeduct: SalaryBreakupEsiPf.ESIDeduct,
          UserId: LoginUserIds,
        },
      ],
      EmpSalaryStructure: SalaryBreakup,
    };
    console.log("payload-----3-", payload);
    const UpsertMaster = JSON.stringify(payload);
    const SalaryStructure = {
      OperationType: "Update",
      JsonData: UpsertMaster,
    };
    console.log("payload------4", SalaryStructure);
    const UpsertEmployeeOfferSalaryStructure = await axios
      .post(
        baseUrl + MethodNames.UpsertEmployeeOfferSalaryStructure,
        SalaryStructure
      )
      .then((res) => {
        console.log("payload------ sucess", res.data);
        if (res.data.UpsertEmployeeOfferSalaryStructure) {
          toast.success("Salary Breakup updated  successfully");
          // router.push("/Screen/EmployeeDetails");
        }
      });
    ViewProjectPayComponents();
  };

  // OfferLetterEmp emp id send
  const OfferLetterEmp = () => {
    router.push(
      {
        pathname: "/Screen/EmpOfferLetter",
        query: {
          //  SalaryBreakupData: JSON.stringify(res.data.UpsertMasterEmployee[0].RecordStatus),
          EmpIdOfferLetter: GetProjectId.EmpId,
        },
      },
      undefined,
      { shallow: true }
    );
  };

  //  back button
  const back = () => {
    router.push("/Screen/NewEmployee");
  };

  console.log("SalaryBreakupEsiP-----0f000", SalaryBreakupEsiPf);

  return (
    <div className="h100">
      <ToastContainer />
      <MainContainer>
        <Sidebaar show={show} />
        <div className={open ? "content-page2 vh-100" : "content-page"}>
          <div className="content  ">
            <div className="container-fluid">
              <div className="cardd">
                <div className="col-sm-12  p-2   ">
                  <div className=" row ">
                    <div className="col-sm-12 col-12 col-md-12   ">
                      <div className=" w-100 back-color table_ref_head">
                        <h3 className="fs_15 back-color table_ref_head text-white p-3">
                          Salary Breakup Add
                        </h3>
                      </div>
                    </div>
                    <div className="col-sm-12 col-12 col-md-12   ">
                      <div className="fs_15 fw-bold table_ref_head mb-1  ">
                        {/* <button
                              type="button"
                              data-bs-toggle="modal" data-bs-target="#staticBackdrop"
                              className="btn btn-primary btn_size fs_12  btn_h_36 m-1 ms-2 m-0 p-0 "
                              >
                           <a className="link_name text-white"
                              // href="./PtaxMasterAdd"
                              >
                           <i className="bx bx-plus ms-1 mt-1"></i> Add New
                           Master
                           </a>
                           </button> */}
                        {/* <button
                           type="button"
                           // data-bs-toggle="modal"
                           // data-bs-target="#exampleModal"
                           // data-bs-whatever="@mdo"
                           onClick={() => addModel()}
                           className="btn btn-primary btn_siz w-auto float-start  fs_12  btn_h_36 m-1 ms-2  "
                           >
                           <i className="bx bx-plus ms-1 mt-1"></i> Add New
                           SalaryBreakup
                           </button> */}
                        <div className=" bg-transparen  cardd float-end btn btn-primar btn_siz w-auto   fs_12  btn_h_36 m-1 ms-2 ">
                          <div className="  text-dark d-flex pt-1 mx-4">
                            <input
                              class="form-check-inpu ms-1 me-2"
                              value={SalaryBreakupEsiPf.PFDeduct}
                              onChange={(e) => {
                                let value = e.target.value == true ? 0 : 1;
                                console.log("valueeeeee", value);
                                setSalaryBreakupEsiPf({
                                  ...SalaryBreakupEsiPf,
                                  PFDeduct: value,
                                });
                              }}
                              checked={
                                SalaryBreakupEsiPf.PFDeduct == 0 ? false : true
                              }
                              type="checkbox"
                            />
                            PF Deduct
                          </div>
                        </div>
                        <div className=" bg-transparen  cardd float-end btn btn-primar btn_siz w-auto   fs_12  btn_h_36 m-1 ms-2 ">
                          <div className="  text-dark d-flex pt-1 mx-4">
                            <input
                              class="form-check-inpu ms-1 me-2"
                              value={SalaryBreakupEsiPf.ESIDeduct}
                              onChange={(e) => {
                                let value = e.target.value == true ? 0 : 1;
                                console.log("valueeeeee", value);
                                setSalaryBreakupEsiPf({
                                  ...SalaryBreakupEsiPf,
                                  ESIDeduct: value,
                                });
                              }}
                              checked={
                                SalaryBreakupEsiPf.ESIDeduct == 0 ? false : true
                              }
                              type="checkbox"
                            />
                            ESI Deduct
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  {ViewProjectPayComponent &&
                  ViewProjectPayComponent.length > 0 ? (
                    <div className="table-responsive">
                      <table className="">
                        <tbody>
                          <tr className="">
                            <th className="text-center mt-3 pt-2">Action</th>
                            {/* 
                              <th className="text-center">Id</th>
                              */}
                            <th className="text-center">Component Name</th>
                            <th className="text-center">Consider For PF </th>
                            <th className="text-center">Consider For ESIC </th>
                            <th className="text-center">Consider For PTAX </th>
                            <th className="text-center">Amount </th>
                          </tr>

                          {Render == Render &&
                            ViewProjectPayComponent &&
                            ViewProjectPayComponent.sort((a,b)=>a.PayCompName.localeCompare(b.PayCompName)).map((i, key) => {
                              console.log("ViewProjectPayComponent---",ViewProjectPayComponent)
                              let ViewProjectPayComponents = ViewProjectPayComponent;
                              const SalaryBreakupSelect = SalaryBreakup;
                              const SalaryBreakupIndex = SalaryBreakupSelect.findIndex((item) => item.PayCompCode == i.PayCompCode );
                              const SalaryBreakupObj =SalaryBreakupIndex >= 0? SalaryBreakupSelect[SalaryBreakupIndex]
                                  : {};
                              const SalaryBreakupChecked =
                                SalaryBreakupIndex >= 0 ? true : false;
                              return (
                                <>
                                  <tr key={key}>
                                    <td>
                                      <p className="m-0 ms-2">
                                        <div className="  d-flex">
                                          <input
                                            class="form-check-inpu ms-1"
                                            value={i.PayCompCode}
                                            onChange={(e) => {
                                              const Value = e.target.value;
                                              console.log("Value", Value);
                                              const Checked = e.target.checked;
                                              console.log("Value", Checked);
                                              if (Checked && !SalaryBreakupChecked ) {
                                                const data = {
                                                  EmpId: GetProjectId.EmpId,
                                                  PayCompCode: i.PayCompCode,
                                                  PayCompValue: i.PayCompValue,
                                                  UserId: UserId,
                                                  // ConsiderFor_PF: i.ConsiderFor_PF,
                                                  // ConsiderFor_ESIC: i.ConsiderFor_PF,
                                                  // ConsiderFor_Ptax: i.ConsiderFor_PF
                                                };
                                                SalaryBreakupSelect.push(data);
                                                setSalaryBreakup(SalaryBreakupSelect);
                                              } else if (!Checked && SalaryBreakupChecked) {
                                                console.log("SalaryBreakupSelect---", Value);
                                                SalaryBreakupSelect = SalaryBreakupSelect.filter((i) => i.PayCompCode !== Value);
                                                setSalaryBreakup(SalaryBreakupSelect);
                                              }
                                              setRender(!Render);
                                             
                                            }}
                                          
                                            checked={SalaryBreakupChecked}
                                            type="checkbox"
                                          />
                                        </div>
                                      </p>
                                    </td>
                                    <td>{i.PayCompName}</td>
                                    <td>{i.ConsiderFor_PF}</td>
                                    <td>{i.ConsiderFor_ESIC}</td>
                                    <td>{i.ConsiderFor_Ptax}</td>
                                    <td>
                                      <input
                                        disabled={ SalaryBreakupIndex >= 0 ? false : true}
                                        className="class"
                                        value={i.PayCompValue}
                                        type="text"
                                        onChange={(e) => {
                                          let TextInput = e.target.value.replace(/[^0-9.\s]/g,"").slice(0,7);

                                          let value = TextInput;
                                          let changeValue = i;
                                          changeValue["PayCompValue"] = value;
                                          ViewProjectPayComponents[key] = changeValue;
                                          setViewProjectPayComponent(
                                            ViewProjectPayComponents
                                          );
                                          if (SalaryBreakupIndex >= 0) {
                                            SalaryBreakupObj["PayCompValue"] =value;
                                            SalaryBreakup[SalaryBreakupIndex] =SalaryBreakupObj;
                                            setSalaryBreakup(SalaryBreakup);
                                            setRender(!Render);
                                          }
                                        }}
                                        name="value"
                                      />
                                    </td>
                                  </tr>
                                </>
                              );
                            })}
                        </tbody>
                      </table>
                      <div className="w-100 h-100 d-flex justify-content-end p-2  my-4">
                        {Object.keys(EditSalaryBreakups).length > 0 ? (
                          <>
                          <button
                          style={{width:"200px"}}
                            className="btn btn-primary fs_12 h-100  btn_size_h   ps-4 pe-4"
                            onClick={handleUpdate}
                          >
                            Update Salary Structure
                          </button>
                          <button
                          type="button"
                          // data-bs-toggle="modal"
                          // data-bs-target="#exampleModal"
                          // data-bs-whatever="@mdo"
                          onClick={() => OfferLetterEmp()}
                          className="btn btn-primary btn_siz w_101 w-auto float-start  fs_12    ms-2  "
                        >
                          View Offer Letter
                        </button>
                        </>
                        ) : (
                          <button
                            className="btn btn-primary h-100  fs_12 w_100 btn_size_h   ps-4 pe-4"
                            onClick={handleSubmit}
                          >
                            Insert Role
                          </button>
                        )}
                        
                        <button
                          // onClick={back}
                          className="btn btn-primary fs_12 bg-danger border-0 h-100 w_100 btn_size_h  ms-2  ps-4 pe-4"
                        >
                          <a
                            className="text-white"
                            href="/Screen/EmployeeDetails"
                          >
                            {" "}
                            Back
                          </a>
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <div class="empty-state">
                        <div class="empty-state__content  mb-5">
                          <div class="empty-state__icon">
                            <img
                              src="data:image/png;base64,"
                              alt=""
                            />
                          </div>
                          <div class="empty-state__message">
                            No records has been added yet.{" "}
                          </div>
                          {/* No records has been added yet. */}
                          {/* <div class="empty-state__help">
                        Add a new record by simpley clicking the button on top left side.

                    </div> */}
                        </div>
                      </div>

                      <div class="credit  mt-3">
                        Illustration by{" "}
                        <a href="" target="_blank">
                          Cpm India
                        </a>
                        .
                      </div>
                    </div>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      </MainContainer>
    </div>
  );
};
export default connect(mapStateToProps, mapDispatchToProps)(SalaryBreakup);
