...
import {LoginUserId, ViewProjectMasters,ViewReportingManager,stateMasterApi} from "../api/AllApi";
...
const EmployeeDetails = (props) => {
  ...
  function exportToExcel(data, fileName) {
    ...
    // let filename= fileName+moment().format('DDMMYYYYHHmmss')+'.xlsx';
...
useEffect(() => {
 ...
 const currentPageData = ViewMasterEmployeeData.slice(startIndex,lastIndex)
  ...
  const MasterEmployeeData = async (projectId) => {
    ...
    let MasterEmployeeDatas = {
      ...
      UserId:LoginUserIds || UserId
    };


    console.log("MasterEmployeeDatas",MasterEmployeeDatas,LoginUserIds)

    await axios.post(baseUrl + MethodNames.ViewMasterEmployee, MasterEmployeeDatas)
      .then((res) => {
        console.log(
          "View PayComponent type DATA----",
          res.data.ViewMasterEmployee
        );
         setProjectSelect(res.data.ViewMasterEmployee),
          setViewMasterEmployeeData(res.data.ViewMasterEmployee);
  ...
  const UserUpdate = (user) => {
    router.push(
      {
        pathname: "/Screen/NewEmployeeAdd",
        query: { EmpId: user.EmpId, ProjectId: user.ProjectId },
      },
      undefined,
      { shallow: false }
    );
  ...
  // serach MasterEmployeeData  user
  const searchItems = (searchValue1, searchValue2) => {
    ...
    if (searchValue1 !== "" || searchValue2 !== "") {
      const filterDepartment = ViewMasterEmployeeData.filter((item) => {
        return (
                ...
                <div className="col-sm-12  p-2   ">
                      ...
                      <div className="fs_15 fw-bold table_ref_head ">
                        ...
                        >
                          ...
                          >
                            ...
                            <i className="bx bx-plus ms-1 mt-1"></i>Add Employee
                          </a>
                        </button>
                        <div className="  float-end mt-1 me-1 btn_size me-2 fs_12  btn_h_3 ">
                          <button
                            className=" p-1 bt mt-1 btn-primary btn_size fs_12 fw-none  "
                            onClick={() =>
                              exportToExcel(ViewMasterEmployeeData, "All_Employee_Data.xls")}
                          >
                            Export All Data
                          </button>
                        </div>

                        <div className="  float-end mt-2 ms-5 w_346   fs_12  btn_h_3  ">
                          Project Select
                          <select
                  ...
                  {ShowItem ? (
                    <div className="table-responsive">
                      <table className="">
                        <tbody>
                          <tr>
                            ...
                            <th className="text-center">Project DOJ </th>
                            <th className="text-center">Aadhar No</th>
                            
                            <th className="text-center">Location Name</th>
                            <th className="text-center">Download Data</th>
                          </tr>
                                      ...
                                      return (
                                        <>
                                          <tr key={key}>
                                            ...
                                            <td style={{textAlign:"center"}}>{i.EmpId}</td>
                                            <td>{i.EmpName}</td>
                                            <td>{i.ProjectName}</td>
                                            <td>{i.Mobile}</td>
                                        ...
                                        <>
                                          <tr key={key}>
                                            ...
                                            <td style={{textAlign:"center"}}>
                                              ...
                                              <button
                                                className=" bt btn-primary btn_siz fs_12"
                                                onClick={() =>
                                                  exportToExcel(
                                                    [i],
                                                    i.EmpName + ".xls"
                     ...
                     defaultCurrent={1}
                     total={ViewMasterEmployeeData.length}
                     />
                     </div>
                     </td>
                     </tr>
                            ...
                            <tr>
                              <td className="p-0 m-0" colSpan={10}>
                                <ErrorPage />
                              </td>
                            </tr>
                          )}
                      ...
                      <div class="empty-state">
                        <div class="empty-state__content  mb-5">
                          ...
                          <div class="empty-state__message">
                            All Records been hidden
                          </div>
                          {/* No records has been added yet. */}
                          <div class="empty-state__help">
                            Show all records by simply clicking the button on
                            top right side.
                          </div>
                        </div>
                      </div>
